...
lakes.set_lake=function(def)
	...
	local c_base = minetest.get_content_id(def.base)
	...
	for x = -rad-y, rad+y do
		...
		and nodes[data[area:index(c.x,gy or c.y-1,c.z)]] then
			...
			if not gy then
				gy = c.y
			elseif y > gy then
				data[area:index(c.x+math.random(1,3),c.y-math.random(2,5),c.z+math.random(1,3))] = c_ignore
				data[area:index(c.x,c.y,c.z)] = c_air
			end

			if (y~=0 and uid == c_source) or def.underground then
				data[area:index(c.x-math.random(1,3),c.y-math.random(2,5),c.z-math.random(1,3))] = c_base
				data[area:index(c.x,c.y,c.z)] = c_source
			else
				local ndef = minetest.registered_items[minetest.get_name_from_content_id(uid)] or {}
				if uid == c_ignore or not ndef.walkable then
					data[area:index(c.x+math.random(1,3),c.y-math.random(2,5),c.z+math.random(1,3))] = c_base
				end
...
lakes.registry_lake("fresh water lakes",{
	spawn_in = "default:dirt_with_grass",
	chance = 1,
	min_y = -50,
	max_y = 50,
	radius = math.random(5,20),
	source = "mindeca:liquid_nebula_water",
	base = "mindeca:sand_stardust",
	in_nodes = {
		"default:dirt",
		"default:dirt_with_grass",
		"default:sand",
		"default:water_source",
		"mindeca:liquid_nebula_water",
	}
})

lakes.registry_lake("nebula water lakes",{
